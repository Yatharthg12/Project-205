    ...
    for row in range(0,3):
        ...
        for col in range(0,9):
            if(platform.system{} == "Darwin"):
                boxButton = Button(
                    gameWindow, font = ("Chalkboard SE", 18), borderwidth=3, pady=23, padx=22, bg = "#fff176", highlightbackground="#fff176",activebackground = "#c5e1a5"
                )
...
def placeNumbers():
    ...
    while counter < len(randomColList):
        ...
        if randomNumber not in currentNumbersList:
            numberBox = ticketGrid[row][colNum]
            numberBox.configure(text = randomNumber, fg = "black")
            currentNumbersList.append(randomNumber)
            counter += 1
